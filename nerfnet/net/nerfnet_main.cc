...
int main(int argc, char** argv) {
  ...
  TCLAP::ValueArg<uint16_t> ce_pin_arg("", "ce_pin",
      ...
      cmd);
  TCLAP::ValueArg<uint16_t> csn_pin_arg("", "csn_pin",
      "Set to the index of the NRF24L01 SPI chip-select pin.", false, 0,
      "index", cmd);
  TCLAP::ValueArg<int> irq_pin_arg("", "irq_pin",
      ...
      cmd);
  TCLAP::ValueArg<int> radio2_ce_pin_arg("", "radio2_ce_pin",
      "Optional second NRF24L01 CE pin. Set to enable dual-radio parallel mode.",
      false, -1, "index", cmd);
  TCLAP::ValueArg<uint16_t> radio2_csn_pin_arg("", "radio2_csn_pin",
      "SPI chip-select pin for the optional second NRF24L01.", false, 1,
      "index", cmd);
  TCLAP::ValueArg<int> radio2_irq_pin_arg("", "radio2_irq_pin",
      "Optional IRQ line for the second NRF24L01.", false, -1, "index", cmd);
  ...
  if (dual_radio_enabled) {
    ...
    CHECK(static_cast<uint16_t>(radio2_ce_pin_arg.getValue()) != ce_pin_arg.getValue(),
        "--radio2_ce_pin must differ from --ce_pin");
    CHECK(radio2_csn_pin_arg.getValue() != csn_pin_arg.getValue(),
        "--radio2_csn_pin must differ from --csn_pin");
  }
  const uint8_t radio2_channel = static_cast<uint8_t>(radio2_channel_raw);
  ...
  if (primary_arg.getValue()) {
    ...
    std::thread radio2_thread;
    if (dual_radio_enabled) {
      radio2_interface = std::make_unique<nerfnet::PrimaryRadioInterface>(
    ...
    std::thread radio2_thread;
    if (dual_radio_enabled) {
      radio2_interface = std::make_unique<nerfnet::SecondaryRadioInterface>(
          static_cast<uint16_t>(radio2_ce_pin_arg.getValue()),
          radio2_csn_pin_arg.getValue(), tunnel_fd,
          primary_addr_arg.getValue(), secondary_addr_arg.getValue(),
